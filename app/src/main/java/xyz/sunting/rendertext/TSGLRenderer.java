...
public class TSGLRenderer implements Renderer {
    ...
    public void onDrawFrame(GL10 unused) {
        ...
        // Render the text
        if (tm != null)
            tm.drawSelf(mtrxProjectionAndView);
    }
    ...
    private void Render(float[] m) {
        ...
        int mPositionHandle = GLES20.glGetAttribLocation(mImageProgram, "vPosition");
        GLES20.glVertexAttribPointer(mPositionHandle, 3, GLES20.GL_FLOAT, false, 0, vertexBuffer);
        GLES20.glEnableVertexAttribArray(mPositionHandle);

        // Get handle to texture coordinates location and load the texture uvs
        int mTexCoordLoc = GLES20.glGetAttribLocation(mImageProgram, "a_texCoord");
        GLES20.glVertexAttribPointer(mTexCoordLoc, 2, GLES20.GL_FLOAT, false, 0, uvBuffer);
        GLES20.glEnableVertexAttribArray(mTexCoordLoc);

        // Get handle to shape's transformation matrix and add our matrix
        int mtrxhandle = GLES20.glGetUniformLocation(mImageProgram, "uMVPMatrix");
        GLES20.glUniformMatrix4fv(mtrxhandle, 1, false, m, 0);

        // Get handle to textures locations
        int mSamplerLoc = GLES20.glGetUniformLocation(mImageProgram, "s_texture");
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        mImageProgram = ShaderUtil.createProgramFromName("image.vshader", "image.fshader");
